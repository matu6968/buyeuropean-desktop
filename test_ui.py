...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test BuyEuropean UI implementations"
    )
    parser.add_argument(
        "--gtk", action="store_true", help="Force GTK4 frontend"
    )
    parser.add_argument(
        "--qt", action="store_true", help="Force Qt6 frontend"
    )
    parser.add_argument(
        "--show-all", action="store_true",
        help="Show both UIs one after another"
    )
...
def run_ui(force_gtk, force_qt):
    ...
    try:
        ...
        try:
            App = get_ui_toolkit(
                force_gtk=force_gtk, force_qt=force_qt
            )
            app = App()
            ui_type = "GTK"
            if force_gtk:
            ...
            print(f"Error: {e}")
            print(
                "Please install either PyGObject "
                "(for GTK4) or PyQt6 "
